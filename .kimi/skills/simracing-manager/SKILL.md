...
Connaissance complète et exhaustive du monorepo `simracing-manager`, à jour au **`v2.2.139`**. Ce fichier est la source de vérité du projet — le tenir à jour à chaque changement d'architecture, d'endpoint, de contrat WebSocket, de build ou de déploiement.
...
- `POST /arcade/:id/photo` (admin, `FileInterceptor`) — upload (PNG/JPEG/WebP, 10 Mo max), redimensionné si besoin (`sharp`, max 1600px, même principe que `BlankingMediaService.resizeImageIfNeeded()`) puis stocké en `bytea` (jamais sur le filesystem).
