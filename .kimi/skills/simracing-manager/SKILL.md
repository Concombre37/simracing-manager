...
Connaissance complète et exhaustive du monorepo `simracing-manager`, à jour au **`v2.2.170`**. Ce fichier est la source de vérité du projet — le tenir à jour à chaque changement d'architecture, d'endpoint, de contrat WebSocket, de build ou de déploiement.

## 0. État des releases agent vérifié (2026-09-08)

Les releases GitHub de l’agent ont été relues jusqu’à `v2.2.169`. La chaîne récente est conservée ici : `v2.2.144` (WoL broadcast), `v2.2.145` (contrôle flotte), `v2.2.146` (multi-interface WoL et exclusion admin), `v2.2.147` (actions parallèles), `v2.2.148`/`v2.2.149` (Drive obligatoire), `v2.2.150` (boucle Lua et diagnostics), `v2.2.151` (helper RSlauncher ViGEm), `v2.2.152` (extraction fiable du helper dans pkg), `v2.2.153` (classement Race robuste et déploiement portable), `v2.2.154` (rôle Spectateur et capture Web), `v2.2.155` (écran Spectateur automatique), `v2.2.156` (capture FFmpeg automatique depuis l’agent), `v2.2.157` (correctif de démarrage du module Spectateur), `v2.2.158` (classement de fin ciblé autour du pilote), `v2.2.159` (protection résultats sans temps valide), `v2.2.160` (classement final depuis les temps historiques BDD), `v2.2.161` (repli historique toutes voitures du circuit), `v2.2.162` (message explicite pour la voiture sans historique), `v2.2.163` (séparation visuelle des groupes podium et pilote), `v2.2.164` (insertion du pilote sans historique pour afficher ses voisins), `v2.2.165` (groupes podium/contexte robustes selon la position), `v2.2.166` (masquage individuel des layouts tablette), `v2.2.167` (fusion des inventaires multi-stations), `v2.2.168` (présence du contenu par station), `v2.2.169` (invalidation du cache quand les fichiers des layouts changent), `v2.2.170` (tarifs public et abonné du catalogue tablette). Les versions antérieures restent documentées dans `CHANGELOG.md` et les notes GitHub ; aucune fonctionnalité agent ne doit être supprimée au motif qu’une version intermédiaire n’a pas de tag.
...
- **Version de référence**: l'agent (`apps/agent/package.json`) — `2.2.170`. Les autres workspaces suivent désormais la même version de release.
...
├── CHANGELOG.md
├── CLAUDE.md              # Copie miroir de ce fichier, chargée automatiquement par Claude Code
...
- **Modules**: `Auth`, `Users`, `Stations`, `Sessions`, `DedicatedServers`, `Agent`, `Dashboard`, `Content`, `ContentPreviews`, `BlankingMedia`, `PowerManagement`, `Telemetry`, `Settings`, `Clients`, `TabletMenuHtml` (v2.2.142, voir gotcha "Fichiers statiques" ci-dessous), `BulkActions` (v2.2.145, actions groupées sur plusieurs postes).
...
- `POST /stations/:id/restart` (admin/technician, v2.2.145) — redémarrage distant via WebSocket agent (`system:restart`, symétrique à `shutdown`)

**`BulkActionsController` (`/api/bulk-actions`, v2.2.145)**

- Alimente la page `/pods-control` ("Contrôle flotte") — actions groupées sur une sélection de POD `simulator`, admin/technicien. Depuis v2.2.146, les stations de rôle `admin` sont refusées côté service même si leur UUID est envoyé directement à l'API. Préfixe **délibérément** `bulk-actions` et non `stations/bulk/...`, pour ne jamais risquer de collision avec les routes `stations/:id/...` du même routeur Express (un segment littéral `bulk` à la place de `:id` serait ambigu selon l'ordre d'enregistrement des modules).
- `POST /bulk-actions/wake` / `/restart` / `/shutdown` / `/blanking-hide` / `/blanking-show` / `/update-agent` / `/sync-content` — body `{ stationIds: string[] }` (au moins un). Chaque station est traitée en best-effort (`Promise.allSettled` dans `BulkActionsService`, une station en échec ne bloque jamais les autres), réponse `{ succeeded: string[], failed: { stationId, error }[] }`. Réutilise tel quel le chemin existant de chaque action à l'unité (`PowerManagementService`/`AgentGateway`) — aucune nouvelle logique de commande, juste l'orchestration groupée.
...
- `GET /external/v1/content` (v2.2.117, champs étendus v2.2.118, tarifs v2.2.170) — `ContentLabelsService.getCatalog()` : catalogue voitures/circuits agrégé sur tous les postes (dédupliqué par `acId`), avec image (`ContentPreview`), tags (`ContentLabel`) et `publicPrice`/`subscriberPrice`. Consommé par `/tablet-menu`.
- `GET /external/v1/categories` (v2.2.132) — `ContentCategoriesService.listGroupedByType()` : liste des catégories configurées (voir `ContentCategoriesController` ci-dessous), scindée `{ cars, tracks }`. Consommé par `/tablet-menu` pour générer ses tuiles de filtre dynamiquement.
- `GET /external/v1/menu` (v2.2.117) — `MenuService.listGrouped()` : carte resto/bar groupée par catégorie. Idem.
- `GET /external/v1/arcade` (v2.2.136) — `ArcadeService.list()` : attractions arcade triées par `sortOrder`. Idem.
...
**`ArcadeController` (`/api/arcade`, v2.2.136)**

- `GET /arcade` (admin/technicien) — attractions triées par `sortOrder` puis `createdAt`, `photoUrl` calculé (`/api/arcade/:id/photo`) seulement si une photo a été uploadée.
- `GET /arcade/:id/photo` — **public, sans auth** (comme `ContentPreviewsController#findOne`), sert le binaire stocké en base.
- `POST/PATCH/DELETE /arcade[/:id]` (admin) — `players`/`kind` en texte libre.
- `POST /arcade/:id/photo` (admin, `FileInterceptor`) — upload (PNG/JPEG/WebP, 10 Mo max), redimensionné si besoin **et converti en WebP** (v2.2.140, `optimizeToWebp()`, max 1600px) puis stocké en `bytea` (jamais sur le filesystem). `photoMimeType` vaut donc toujours `image/webp` pour toute photo uploadée depuis v2.2.140.
- `DELETE /arcade/:id/photo` (admin) — retire juste la photo, garde l'attraction.
- Consommé par la page admin `Arcade.tsx` (`/arcade`) et, en lecture, par `/tablet-menu` via `ExternalApiController`.
...
- `PUT /content/labels` (admin) — upsert `{ type: 'car'|'track', acId, displayName, category?, difficulty?, year?, country?, countryCode?, description?, publicPrice?, subscriberPrice? }`. Les prix sont des textes libres de 30 caractères maximum et sont affichés séparément au public et à l'abonné. Jusqu'en v2.2.116, `displayName` vide supprimait la ligne (retour au nom technique) ; **depuis v2.2.117 (étendu v2.2.118), la ligne n'est supprimée que si tous les champs sont vides à la fois** — sinon vider juste le nom personnalisé effacerait aussi le reste déjà renseigné (`ContentLabelsService.upsert()`).
...
**`ContentCategoriesController` (`/api/content-categories`, admin uniquement, v2.2.132)**

- Demandé par l'utilisateur ("j'aimerais que les catégorie soit configurable dans l'admin et que la catégorie soit sélectionnable en liste pour envoyer la voiture dans la catégorie") — remplace le texte libre historique de `ContentLabel.category` (source de quasi-doublons, voir 3.2) par une vraie liste gérée.
- `GET ?type=car|track` — liste triée par `sortOrder` puis `name`.
- `POST` (`{type, name, sortOrder?}`) — 409 si `(type, name)` existe déjà.
- `PATCH /:id` (`{name?, sortOrder?}`) — **renommer une catégorie met aussi à jour tous les `ContentLabel` qui la référencent** (`contentLabel.updateMany` dans la même transaction) : sinon les voitures déjà taguées redeviendraient invisibles pour le nouveau nom.
- `DELETE /:id` — **détache aussi les voitures/circuits qui l'utilisaient** (`category` remis à `null`) plutôt que de laisser une valeur orpheline absente de la liste.
- `type` n'est jamais modifiable après création (changer le type reviendrait à recréer une autre catégorie).
...
- **`ContentPreview`**: `id, stationId (FK), type, acId, name, data (base64)`, unique sur `(stationId, type, acId)`. `type` est une chaîne libre, pas un enum : `car`, `track`, `track-layout` (preview par variante de circuit multi-layout, v2.2.56) et depuis v2.2.128 `layout` (le vrai schéma de tracé `outline.png`, `acId` = celui du circuit — voir 4.2 "Schéma de circuit"). **Depuis v2.2.140, `data` est toujours du WebP** (converti à l'ingestion par `StationsService.upsertPreview()`, voir `common/image-optimizer.ts`) — les 1733 lignes existantes (PNG/JPEG) ont été reconverties en place par un script one-off (449 Mo → 65 Mo, -85,6%). Pas de colonne mimeType dédiée : le type réel est deviné par signature base64 (`inferMimeFromBase64()`, `common/infer-mime.ts`, partagé avec `ContentLabel.layoutImage`). **Depuis v2.2.141, `upsertPreview()` cappe aussi les dimensions (`maxDimension: 2000`)** — la conversion v2.2.140 recompressait sans jamais redimensionner, certaines captures voiture atteignaient 4088×2300px pour un affichage qui ne dépasse jamais ~1180px CSS sur `/tablet-menu` ; les 1792 lignes existantes ont subi une **deuxième** migration one-off, relisant les octets pré-WebP originaux depuis la sauvegarde `pg_dump` de v2.2.140 (restaurée temporairement dans une base scratch) plutôt que de repartir du WebP déjà compressé, pour éviter une double perte de qualité.
- **`ContentLabel`** (v2.2.68) : `id, type ('car'|'track'), acId, displayName, category? (texte libre, ex "GT3", v2.2.117), difficulty? (1-5, v2.2.117), year? (v2.2.118), country?/countryCode? (nom + ISO 3166-1 alpha-2 pour l'emoji drapeau, v2.2.118), description? (v2.2.118), powerHp?/weightKg? (chevaux/kg, v2.2.120), maxSpeedKmh? (v2.2.130), mirrored (booléen, défaut false, v2.2.123 — miroir horizontal CSS appliqué sur /tablet-menu, jamais un retournement des pixels stockés, qui serait annulé à la prochaine resynchronisation de contenu depuis un poste), visible (booléen, défaut true, v2.2.124 — décoché retire l'item du catalogue public /tablet-menu sans toucher au reste de ses données), layoutImage? (base64, v2.2.125 — schéma réel d'un circuit vu du dessus, circuits uniquement, jamais via le formulaire admin, voir 4.2), createdAt, updatedAt`, unique sur `(type, acId)`. **Global** (pas de FK station — un `acId` désigne le même contenu AC partout), contrairement à `ContentPreview` qui est scopé par station. Tous ces tags alimentent la page tablette (`/tablet-menu`) — pas de valeur inventée, `null` tant que l'admin n'a rien renseigné via `/content-names`. **259 des 346 voitures scannées en prod ont été peuplées en masse le 2026-08-11/12** (recherche des vraies données par catégorie/année/pays/description/puissance/poids, difficulté estimée par catégorie) via des scripts Prisma one-off — volontairement incomplet, les mods dont le constructeur est déguisé pour raison de licence (contenus RSS "Bayer"/"Lanzo"/"Protech"...) ont été exclus plutôt que devinés. **66 des 87 voitures encore vides ont été peuplées le 2026-08-12 (v2.2.130)** à partir d'un dump SQL `elsass_catalogue` fourni par l'utilisateur (même nom que le venue — vraisemblablement un catalogue existant du même roster) : correspondance acId↔ligne faite à la main (le dump n'a pas d'acId, seulement un nom libre), en décodant au cas par cas les constructeurs déguisés RSS via le `rawName` réellement scanné (souvent déjà en clair, ex: `rss_gtm_bayer_i6_evo` → rawName scanné "GT3 BMW M4") et les abréviations de modèle ("Fera 55/57"→Ferrari 550/575, "Adonis D9"→Aston Martin DBR9...) — 21 restent explicitement exclues (plusieurs lignes du dump également plausibles, ou aucune ligne correspondante). Seuls les champs encore `null` ont été renseignés, jamais une valeur déjà curatée écrasée. **63 des 68 circuits scannés en prod ont été peuplés le 2026-08-12** (pays/code pays/difficulté, 59 avec un vrai schéma de tracé `layoutImage`) via une recherche web parallélisée (5 agents) — les circuits fictifs Kunos ou aux noms trop génériques pour être identifiés avec confiance ont été exclus plutôt que devinés (voir 4.2). **Catégories voitures nettoyées le 2026-08-12 (v2.2.132), 88 → 48**, à la demande explicite de l'utilisateur après avoir vu l'explosion de tuiles de filtre que la migration vers `ContentCategory` a révélée sur `/tablet-menu` : fusion des 4 doublons de casse/langue évidents (`Formule 1`→`Formula 1`, `Formule 4`→`Formula 4`, `LMP2`→`Prototype LMP2`, `CUP`→`Cup`), suppression de la catégorie `Hors Catégories` (redondante avec l'absence de catégorie), et détachement des 35 catégories qui ne s'appliquaient qu'à une seule voiture après fusion (`category` remis à `null`, aucun autre champ touché) — les vraies sous-catégories à faible effectif mais réelles (`NASCAR`, `WRC`, count=2 chacune, etc.) ont été conservées, aucune décision de fusion incertaine (ex: `GT1` vs `GT1 / Le Mans`, les variantes "historique"/"préparé" de chaque famille) n'a été prise unilatéralement.
- **`ContentCategory`** (v2.2.132) : `id, type ('car'|'track'), name, sortOrder (défaut 0), createdAt, updatedAt`, unique sur `(type, name)`. Liste de catégories configurable via `/content-categories` (admin) — remplace le texte libre historique de `ContentLabel.category`, gérée par `ContentCategoriesController`/`ContentCategoriesService`. **Seedée à la création (migration `20260812150000_add_content_categories`) à partir des 88 valeurs de `ContentLabel.category` déjà en base** (aucune catégorie inventée ni fusionnée à ce stade, triées par fréquence d'usage décroissante) — voir 3.2 `ContentLabel` pour le nettoyage manuel qui a suivi (88 → 48). Renommer/supprimer une catégorie répercute la même opération sur les `ContentLabel` qui la référencent (voir `ContentCategoriesController` en 3.1).
- **`MenuCategory`/`MenuItem`** (v2.2.117, tarif abonné v2.2.170) : carte resto/bar affichée sur `/tablet-menu`, gérée via `/restaurant-menu`. `MenuCategory`: `id, section ('food'|'drinks'), title, subtitle?, sortOrder (défaut 0)`, relation `items[]`. `MenuItem`: `id, categoryId (FK MenuCategory, cascade), name, description?, price` public et `subscriberPrice?` (textes libres, pas de calcul), `sortOrder (défaut 0)`. Rien n'est seedé — vide jusqu'à ce que l'admin remplisse la carte lui-même.
- **`ArcadeAttraction`** (v2.2.136) : `id, name, players? (texte libre "2 à 4 joueurs"), kind? (texte libre "Borne à jetons"), photo? (bytea), photoMimeType?, sortOrder (défaut 0), createdAt, updatedAt`. Affichée sur l'onglet "Arcade" de `/tablet-menu`, gérée via `/arcade`. **Seule table de contenu tablette dont la photo est uploadée à la main** (billard/baby-foot/bornes n'ont aucune source de scan automatique, contrairement aux voitures/circuits) — stockée en base comme `BlankingMedia.data`, jamais sur le filesystem. Rien n'est seedé — vide jusqu'à ce que l'admin remplisse la liste, même principe que `MenuCategory`. Demandé par l'utilisateur après avoir remarqué que l'onglet Arcade de la maquette v2 (v2.2.134) avait été volontairement omis faute de vraie donnée ("Tu as oublié arcade") — commissionné comme vraie fonctionnalité plutôt que réaffiché avec du contenu inventé.
...
- **Rôles station (`simulator`, `admin`, `spectator`)**: `Station.role` (enum `StationRole` dans `@simracing/shared`), défaut `simulator`. **Simulator** = POD joueur (peut lancer en direct, peut rejoindre un serveur dédié). **Admin** = PC hébergeur uniquement (peut héberger `acServer.exe`, ne peut ni lancer ni rejoindre). **Spectator** = poste de suivi/capture Web, exclu de toute commande POD et du blanking. Le frontend et le backend réappliquent ces frontières, donc impossible à contourner via appel API direct.
- **`import sharp from 'sharp'` compile en un appel qui vaut `undefined` à l'exécution, avalé en silence par un try/catch — trouvé en 2026-08-22 en déboguant la conversion WebP (v2.2.140)** : ce projet n'active ni `esModuleInterop` ni `moduleResolution: "node16"/"bundler"` dans `tsconfig.json` (backend). Sans `esModuleInterop`, TS compile `import sharp from 'sharp'` en `sharp_1.default(...)` — `sharp` exporte via `export =` (pas de default ESM), donc `.default` vaut `undefined` et l'appel jette un `TypeError` à l'exécution. **`BlankingMediaService.resizeImageIfNeeded()` avait exactement cet import depuis son ajout (v2.2.95) et avalait ce `TypeError` dans son propre try/catch de repli** — le redimensionnement des images de blanking surdimensionnées ne s'est donc **jamais** exécuté en production, sans aucun symptôme visible (l'upload réussit quand même, juste jamais optimisé). Corrigé partout (`blanking-media.service.ts`, nouveau `common/image-optimizer.ts`) via `import sharp = require('sharp')` (la syntaxe correcte pour ce style d'export). **Le typage reste un souci séparé** : même corrigé à l'exécution, TS résout encore les mauvaises déclarations du paquet (fichiers ESM `.d.mts` au lieu des `.d.cts` réellement utilisés en CommonJS) faute de `moduleResolution` moderne — contourné par un retypage local minimal (`MinimalSharp`) dans ces deux fichiers plutôt qu'un changement de configuration TS global, qui aurait un rayon d'effet imprévisible sur tout le reste du backend et dépasse le cadre de ce correctif.
...
| `/content-categories`             | `ContentCategories`     | `ProtectedRoute`, admin       | CRUD liste de catégories voitures/circuits, v2.2.132 — voir 4.2   |
...
| `/arcade`                         | `Arcade`                | `ProtectedRoute`, admin       | CRUD attractions arcade + photo, v2.2.136 — voir 4.2              |
...
| `/pods-control`                   | `PodsControl`           | `ProtectedRoute`, admin       | actions groupées sur la flotte, v2.2.145 — voir 4.2               |
...
- **`ContentNames.tsx`** (`/content-names`, admin, v2.2.68, étendu v2.2.117 puis v2.2.118/v2.2.120, lisibilité revue v2.2.122, bascule miroir v2.2.123, case visibilité catalogue v2.2.124, miniature photo v2.2.127, vignette Layout v2.2.128, galerie multi-layout + fond noir v2.2.129, champ vitesse max v2.2.130, catégorie en liste v2.2.132, bouton "Voitures sans infos" v2.2.133) : **depuis v2.2.133**, un bouton dans l'en-tête (`isMissingInfo()`, module-level) filtre la liste sur les voitures n'ayant **aucune** info technique/descriptive (ni catégorie, difficulté, année, pays/code pays, description, puissance, poids, vitesse max — le nom affiché seul ne compte pas) ; force le filtre Type sur "Voitures" en même temps, un second clic restaure la liste complète. En plus du nom personnalisé, chaque ligne a un champ **catégorie** — **depuis v2.2.132, un `<select>`** alimenté par `GET /content-categories` (filtré par `item.type`, plus de texte libre — demandé par l'utilisateur, "que la catégorie soit sélectionnable en liste pour envoyer la voiture dans la catégorie"), avec un bouton "Gérer les catégories" dans l'en-tête de page qui ouvre `/content-categories`. Un sélecteur **difficulté** (5 barres cliquables, 1-5, cliquer la valeur déjà sélectionnée l'efface), **année**, **pays** (texte libre) + **code pays** (2 lettres ISO, affiche l'emoji drapeau live via `flagEmoji()` — combinaison Unicode de deux "regional indicator symbols", pas d'image), **description** courte, et depuis v2.2.120 **puissance (ch)**/**poids (kg)** (deux inputs numériques), plus depuis v2.2.130 **vitesse max (km/h)**. Un seul bouton "Enregistrer" par ligne envoie tous les champs ensemble (`ContentNameRow`, state local par champ) — nécessaire depuis que `upsert()` ne supprime plus la ligne sur un nom vide seul (voir 3.1/3.3). **Depuis v2.2.122, chaque champ a une étiquette permanente au-dessus** (`Field`, petit composant label+enfant) plutôt que de compter uniquement sur le `placeholder` — demandé par l'utilisateur ("plus lisible") après avoir cherché où éditer ces infos : le placeholder disparaît dès qu'une valeur est saisie, ce qui rendait les lignes déjà remplies illisibles sur une liste de 346 voitures ; layout en grille (`grid-cols-2 sm:grid-cols-3 lg:grid-cols-6`) plutôt qu'un flex-wrap improvisé. Depuis v2.2.123, une ligne voiture gagne aussi un bouton bascule **Miroir** (`FlipHorizontal`) pour corriger l'orientation de la photo sur `/tablet-menu` au cas par cas. Depuis v2.2.124, une case à cocher **Afficher sur le catalogue** (toutes lignes, voitures et circuits) contrôle la présence de l'item sur `/tablet-menu` sans effacer le reste de ses données. Depuis v2.2.127, une petite vignette (56×40) avec la vraie photo scannée précède le badge de type sur chaque ligne — demandé par l'utilisateur ("petit mais visible la photo de la voiture") pour identifier visuellement la ligne en cours d'édition sans deviner d'après le seul acId ; icône de remplacement si aucune preview n'a encore été scannée. Depuis v2.2.128, une ligne circuit gagne à la place du bouton Miroir un champ **Layout** en lecture seule montrant le vrai schéma de tracé scanné par l'agent, ou "Aucun" tant qu'aucun poste avec le nouvel agent n'a resynchronisé ce circuit — depuis v2.2.129, vignette sur **fond noir** (le trait du schéma scanné est blanc, invisible sur fond blanc) et **une vignette par layout nommé** du circuit (galerie) quand il y en a plusieurs, avec le nom du layout en infobulle. Tous ces champs alimentent la page tablette publique `/tablet-menu`.
- **`ContentCategories.tsx`** (`/content-categories`, admin, v2.2.132) : CRUD de la liste de catégories consommée par le `<select>` de `ContentNames.tsx` et les filtres de `/tablet-menu` — deux colonnes (Voitures/Circuits, `ContentCategoriesService.list(type)`), chacune une liste triée par `sortOrder` avec renommage inline (`onBlur`), boutons monter/descendre (échange `sortOrder` entre voisins, deux `PATCH`), suppression avec confirmation inline, et un champ d'ajout en bas. Filtre texte par colonne (88 catégories voitures au premier chargement, une simple molette ne suffit pas). Renommer/supprimer répercute côté backend sur tous les `ContentLabel` déjà tagués (voir 3.1) — les deux mutations invalident donc aussi `['content-labels-known']`, pas juste `['content-categories']`.
...
- **`Arcade.tsx`** (`/arcade`, admin, v2.2.136) : CRUD des attractions arcade — grille de cartes, chacune avec sa photo (upload/remplacement/suppression via un `<input type="file">` caché déclenché par un bouton flottant sur la vignette, pas besoin d'ouvrir la modale pour changer juste la photo), nom, joueurs, type. Bouton "Nouvelle attraction" ouvrant une `Modal` de formulaire (nom/joueurs/type seulement — pas de champ photo dans le formulaire, l'upload se fait après création directement sur la carte). Pas de champ de tri exposé, même convention que `Menu.tsx`.
- **`TabletMenu.tsx`** (`/tablet-menu`, **page publique sans compte**, v2.2.117, enrichie v2.2.118, thème "Nocturne" remplacé par l'identité réelle du site vitrine en v2.2.134, onglet Arcade en v2.2.136, vrai logo en v2.2.137/v2.2.138, agrandi en v2.2.139, performance mobile revue en v2.2.141/v2.2.142) : réimplémentation React d'une maquette Claude Design importée. 5 onglets (Voitures/Circuits/Cuisine/Bar/Arcade), filtres catégorie **dynamiques depuis v2.2.132** (générés depuis `GET /external/v1/categories`, matché exact sur `item.category` — la barre ne s'affiche que s'il y a au moins un item pour une catégorie donnée), fiche détail au clic (photo + catégorie + difficulté + depuis v2.2.118 pays/drapeau/année/description quand renseignés — **pas** de puissance/poids inventés), écran de veille après `IDLE_MS` (90s) d'inactivité (accroche tournante calculée depuis les vrais compteurs), portrait/paysage détecté via `window.innerWidth`/`innerHeight` (`vh > vw*1.05`). **Depuis v2.2.134**, thème repris de l'identité réelle du site vitrine du client (`elsass-simracing.fr`) plutôt que de la maquette générique d'origine : fond anthracite `#0E0E0E`/`#242424`, accent bleu acier `#245E97`, police Montserrat — tokens CSS scopés à `.tablet-menu` (`TabletMenuStyles`), jamais sur `:root`, pour ne pas teinter le reste du dashboard. Navigation en barre basse unique dans les deux orientations (plus de rail latéral en paysage). **Pas de toggle FR/EN** — tout le reste du site est 100% français sans infra i18n. Consomme `GET /external/v1/content`, `/categories`, `/menu` et `/arcade` via `services/tabletMenu.ts` → `services/externalApi.ts` (voir gotcha ci-dessous). **Depuis v2.2.138, le vrai logo (`/logo-elsass-simracing.svg`) remplace le wordmark texte partout sur cette page** (en-tête persistant des 5 onglets **et** écran de veille) — pas de compte utilisateur sur cette page donc pas d'accès à `useSiteLogo()` (endpoint JWT/clé station uniquement), d'où un asset statique bundlé plutôt qu'un logo configurable en base.
  - **v2.2.142, demandé par l'utilisateur (deuxième rapport PageSpeed, "network-dependency-tree-insight" encore signalé)** : `TabletMenuHtmlController` (voir 3, "Fichiers statiques") précharge le chunk JS de la page depuis son propre `index.html` — vérifié au niveau réseau, le chunk démarre désormais son téléchargement en parallèle du bundle principal plutôt qu'après son exécution.
  - **v2.2.141, demandé par l'utilisateur (rapport PageSpeed Insights mobile, "j'ai pas encore 100%")** : score de performance mobile 55→89. Le logo de l'écran de veille (`IdleScreen`, élément LCP de la page) gagne `fetchPriority="high"` + un `<link rel="preload" as="image" fetchpriority="high">` dans `index.html` (sinon invisible pour le navigateur tant que le bundle JS n'est pas exécuté, contrairement à une `<img>` statique du HTML) ; les images de grille (`CatalogCard` voitures/circuits/tracés, `ArcadeCard`) passent en `loading="lazy"` (hors écran au premier rendu). Voir aussi `ContentPreview` (3.2) pour le recadrage `maxDimension` de la même passe, et `App.tsx` (gotcha ci-dessous) pour le découpage du bundle JS qui l'accompagne.
  - **v2.2.138, demandé par l'utilisateur ("Utilise le bon logo pour les catalogue menu" + "faut juste écrire la difficulté en haut a droite des images")** : l'en-tête persistant (visible en naviguant les 5 onglets, contrairement à `IdleScreen` seul en v2.2.137) passe du wordmark texte/CSS (`VENUE_WORD_1`/`VENUE_WORD_2`/`VENUE_CITY`) au même `<img>` vers le logo SVG, redimensionné (32px/44px de haut). `CatalogCard` (voitures et circuits) gagne un badge texte de difficulté (`DIFFICULTY_LABELS`) en haut-droite de l'image quand elle est renseignée, remplaçant l'ancien indicateur à points (`DifficultyDots`) qui n'apparaissait qu'en bas des cartes circuit (les cartes voiture n'en avaient aucun sur la tuile auparavant) ; le badge "N tracés" des circuits multi-layout est déplacé en haut-gauche pour laisser la place. `DetailModal` garde `DifficultyDots` (points + libellé), inchangée.
  - **v2.2.137, demandé par l'utilisateur** (a fourni le vrai fichier logo — un PDF vectoriel Illustrator "ES.pdf", monogramme + wordmark "ELSASS SIMRACING HAGUENAU" — via une URL directe après clarification, la pièce jointe collée dans la conversation n'étant pas accessible sur le disque du serveur) : converti en SVG vectoriel (`pdftocairo -svg`, transparence native, aucune perte) plutôt que rasterisé, servi en asset statique (`apps/frontend/public/logo-elsass-simracing.svg`, même convention que `vite.svg`). Le bloc "ELSASS/SIMRACING"+"HAGUENAU" de l'écran de veille (texte + traits CSS) est remplacé par une simple `<img>` vers ce SVG. Couleurs du fichier fourni conservées telles quelles (bleu de marque légèrement plus saturé que `--tm-accent`), pas forcées sur les tokens CSS existants.
  - **v2.2.136, demandé par l'utilisateur ("Tu as oublié arcade")** : l'onglet "Arcade" de la maquette v2 avait été volontairement omis en v2.2.134 faute de vraie donnée derrière — commissionné cette fois comme une vraie fonctionnalité (nouveau modèle `ArcadeAttraction`, module backend `arcade`, page admin `/arcade`, voir 3.1/3.2/4.2) plutôt que réaffiché avec du contenu inventé. `ArcadeCard` : grille de présentation **sans fiche détail au clic** (pas de modale, contrairement aux voitures/circuits — simple grille comme dans la maquette), photo en couleur (même `--tm-photo` que le reste), état vide honnête tant qu'aucune attraction n'est configurée. Icône de nav joystick reprise de la maquette (`NavIconArcade`).
  - **v2.2.134, demandé par l'utilisateur ("j'aimerais refaire la page du /tablet-menu... j'aimerais avoir le maximum de beauté [du vrai site elsass-simracing.fr]")** : nouveau prompt de conception écrit à partir d'un rendu Playwright du vrai site (couleurs/police extraites du CSS calculé — `#245E97`/`#242424`/Montserrat — plutôt que devinées), nouvelle maquette Claude Design importée ("Simracing tablet menu", `Kiosque Elsass Simracing.dc.html`) et réimplémentée avec les mêmes écarts assumés que toutes les maquettes précédentes de ce projet : (1) la maquette ajoutait deux onglets fictifs "Arcade" et "Le centre" (bornes d'arcade, photos de salle, événements à venir) sans aucune donnée réelle derrière — **non repris à ce stade**, contrairement aux 4 onglets réellement demandés dans le prompt de conception (l'onglet Arcade a depuis été commissionné pour de vrai en v2.2.136, voir plus haut ; "Le centre" reste sans donnée réelle) ; (2) les circuits de la maquette étaient des tracés SVG inventés avec longueur/virages/sens fictifs — remplacés par le vrai `outline.png` scanné (v2.2.128) sur un panneau noir à halo radial, aucune spec fabriquée ; (3) les drapeaux SVG dessinés à la main pour 9 pays fixes ont été laissés de côté au profit de `flagEmoji()` déjà en place (couvre n'importe quel code pays réel) ; (4) l'écran de veille de la maquette utilisait une photo plein écran du centre (`<image-slot>`, aucun asset réel de ce type côté serveur) — remplacé par un halo radial bleu, sans photo inventée ; (5) les cartes voiture de la maquette étaient volontairement sans photo (dégradé + filigrane de catégorie) — la vraie photo scannée déjà curatée (v2.2.123) a été **conservée** en fond de carte plutôt que supprimée, pour ne pas perdre ce travail ni la reconnaissance visuelle.
  - **v2.2.135, demandé par l'utilisateur immédiatement après ("il faut la couleur sur le image pas noir et blanc sur fond bleu")** : le filtre "duotone bleu" initial (`grayscale + sepia + hue-rotate`, repris de l'option par défaut de la maquette) rendait les photos trop proches du noir & blanc teinté — remplacé par `--tm-photo` (`contrast`/`brightness`/`saturate` seulement, vraies couleurs conservées). Voiles bleus de marque au-dessus des photos (tuiles, cartes, reflet de `Tilt3DImage`) nettement réduits en opacité — gardés comme légère cohérence de marque, plus comme teinte dominante ; le reflet dynamique de `Tilt3DImage` est passé de bleu à blanc neutre (effet verre).
...
- **Sélecteur de catégories à tuiles (`FamilyTile` v2.2.119 → `CategoryTile`/`PillFilter` v2.2.134, liste remplacée en v2.2.121, rendue dynamique en v2.2.132)** : demandé par l'utilisateur à partir d'une image de référence (4 tuiles photo GT/Formula/LMDH/Drift) puis remplacé par une liste de 11 catégories figées, puis (v2.2.132, "j'aimerais que les catégorie soit configurable dans l'admin") rendu dynamique — les tuiles sont générées depuis la vraie liste `ContentCategory` (`GET /external/v1/categories`), une par catégorie configurée ayant au moins un item, filtre exact sur `item.category === cat.name`. `filter` (state) stocke le **nom** de la catégorie directement. **Plus de bucket "Autres"** : un item sans catégorie reste visible sous "Toutes" sans tuile dédiée. **Depuis v2.2.134, deux composants distincts** (repris de la maquette v2 du vrai site vitrine) au lieu d'un seul `FamilyTile` partagé : `CategoryTile` (voitures — grande tuile 250×108, photo réelle en couleur + léger voile diagonal bleu de marque si une preview existe pour la catégorie, sinon tuile pleine avec un simple trait d'accent) et `PillFilter` (circuits — pastille compacte sans photo, un circuit n'a pas de "photo de catégorie" au sens propre). Chaque tuile/pastille affiche le nombre de véhicules/circuits correspondants.
...
- **Schéma de circuit (`ContentLabel.layoutImage`, v2.2.125, peuplé v2.2.126, remplacé par un vrai scan agent en v2.2.128)** : demandé par l'utilisateur ("pour les circuit récupère les vrais schema du circuit"). Champ pour le vrai tracé du circuit vu du dessus (pas la photo scannée par l'agent), servi via `GET /content/labels/layout-image/:id` (public, sans auth, même principe que `ContentPreviewsController`). Renseigné uniquement par script one-off (recherche web, sources libres type Wikimedia Commons) — pas de champ dans le formulaire `/content-names`, `upsert()` ne le touche jamais. **59/68 circuits peuplés** ainsi (recherche parallélisée sur Wikimedia Commons, images normalisées en PNG max 900px, vérifiées visuellement une par une) — les 5 circuits fictifs/non identifiables et les 4 circuits réels sans schéma en licence libre disponible n'ont pas reçu d'image inventée, voir 3.2.
  ...
  - **v2.2.132, demandé par l'utilisateur ("dans l'image du circuit dans le catalogue j'aimerais un truc propre mais il me faut le tracé du circuit")** : la tuile circuit de la grille `/tablet-menu` (`CatalogCard`) affichait encore la photo de piste (`preview`, souvent sombre/encombrée, coupée en `object-fit: cover`) au lieu du tracé — seule la fiche détail avait été mise à jour en v2.2.129. `CatalogCard` affiche désormais le tracé (`layoutImages[0]` ou repli `layoutImageUrl`) en priorité sur la photo, fond noir + `object-fit: contain` ; repli photo si aucun tracé, puis icône "pas d'image". Les voitures (`layoutImages` toujours vide) ne sont pas concernées, leur tuile garde la photo.
- **`App.tsx` — toutes les pages en `React.lazy()` + `<Suspense>` depuis v2.2.141**, pas des imports statiques comme avant : `/tablet-menu` est une route publique sans compte consultée depuis des tablettes clientes, mais avec des imports statiques elle embarquait quand même tout le code de chaque page admin dans le même bundle initial (Dashboard, Stations, Users, Arcade...) — repéré via un audit PageSpeed Insights mobile (148 Ko de JS jamais utilisé sur cette page). Chaque page est désormais son propre chunk Vite, chargé à la demande ; `RouteFallback` (composant module-level, même style que le loader de `ProtectedRoute`) sert de fallback `<Suspense>` pendant le téléchargement du chunk. Toute nouvelle page ajoutée à ce fichier doit suivre le même pattern (`lazy(() => import('./pages/X').then((m) => ({ default: m.X })))`) plutôt qu'un import statique, pour ne pas réintroduire le problème.
...
- **`PodsControl.tsx`** (`/pods-control`, admin, v2.2.145, amélioré v2.2.146) : grille limitée aux stations de rôle `simulator`, toutes sélectionnées par défaut ; les postes admin ne sont jamais affichés ni comptés. Sélections rapides Tous/En ligne/Hors ligne/Aucun, POD connectés triés en premier, état/version/IP visibles sur chaque carte et actions désactivées sans sélection. Trois cartes d'actions groupées (`services/bulkActions.ts` → `BulkActionsController`) : Alimentation (Allumer/Redémarrer/Éteindre la sélection — les deux derniers avec confirmation native), Écrans d'attente (Masquer/Réafficher), Maintenance (Mettre à jour/Synchroniser). Chaque bouton affiche un résumé succès/échecs par poste après coup (`BulkActionResult`). Live-update via `station:updated` câblé en `useEffect`.
- **Dashboard de contrôle rapide (`FleetQuickControl.tsx`, v2.2.147)** : la page `/` permet de sélectionner Tous/En ligne/Hors ligne/Aucun ou des POD individuels, puis d'allumer, redémarrer, éteindre, masquer ou afficher le blanking directement. La portée est calculée avant l'envoi : WoL seulement pour les POD hors ligne, autres commandes seulement pour les POD joignables ; le nombre de cibles est affiché sur chaque bouton. Les admins sont filtrés côté composant en plus du verrou backend. Chaque POD montre connexion et état du blanking, avec retour de succès/erreur après action.
- **Lancements/arrêts simultanés depuis `/stations` (v2.2.147)** : ne jamais utiliser un unique `useMutation().isPending` pour désactiver les boutons de toutes les lignes. `launchingIds` et `stoppingIds` suivent chaque UUID indépendamment, ce qui autorise 1 à 15 requêtes concurrentes ; seul le POD dont la requête est en cours est verrouillé. Une erreur API indique le nom du POD concerné.
...
- **Mise en Drive (v2.2.150)** : ne jamais lire `sim.isSessionStarted` dans Lua : ce nom appartient à notre payload Node, pas à une API CSP vérifiée. La boucle utilise `sim.isInMainMenu` et `os.preciseClock()` (le `dt` du menu peut être nul), retente `ac.tryToStart()` toutes les 500 ms et conserve le drapeau pendant une seconde stable hors menu avec voiture présente. En lancement direct, armer après taskkill de l'ancien processus. Les IDs des commandes sont des UUID uniques entre instances LuaBridge. Les logs du site ajoutent les diagnostics Lua horodatés (`lua_loaded.txt`, `status.txt`, `lua_error.txt`, `join.flag`) ; une commande écrite ne prouve jamais son exécution. Tests du vrai script dans `apps/agent/scripts/test-lua.py` avec LuaJIT/Lupa 2.6, exécutés en CI. `readSessionId` et `sendHttpTelemetry` doivent être déclarés localement avant les fonctions qui les appellent. Le blanking conserve son mécanisme existant ; ne pas confondre mémoire partagée fraîche et accusé de réception Drive. La validation sur POD doit contrôler les diagnostics et le comportement en jeu.
...
- Heartbeat inclut `macAddress`. WoL envoyé sur les ports 9 et 7. **Toujours en broadcast dirigé du sous-réseau du relais (`getBroadcastAddress(targetIp)`), jamais en unicast vers l'IP de la cible**. Depuis v2.2.146, l'interface réseau du relais est choisie d'après le sous-réseau de `targetIp` (masque le plus précis en cas de plusieurs correspondances) et le broadcast est calculé depuis son vrai netmask, au lieu de prendre la première interface IPv4 et de supposer `/24`.
- **`system:restart` était déclaré dans le contrat partagé mais jamais branché nulle part avant v2.2.145** — trouvé en implémentant la page `/pods-control` (demande utilisateur "renforcer le redémarrage possible des POD"). Seul `system:shutdown` avait un vrai handler (`agent.ts#handleShutdown()`, `shutdown /s /t 0`) ; `system:restart` figurait dans `ServerToAgentEvents` (`packages/shared`) depuis longtemps sans qu'aucun code (agent, `AgentGateway`, contrôleur, bouton frontend) ne l'émette ou ne l'écoute — un redémarrage distant était donc silencieusement impossible, seul un arrêt suivi d'un réveil WoL séparé pouvait l'imiter. Câblé de bout en bout : `agent.ts#handleRestart()` (`shutdown /r /t 0`, symétrique), `AgentGateway.emitRestart()`, `PowerManagementService.restart()`, `POST /api/stations/:id/restart`.
...
- [ ] Redémarrage distant relance bien le POD Windows cible (v2.2.145).
- [ ] `/pods-control` : les actions groupées respectent la sélection (un poste décoché n'est jamais touché), et le résumé succès/échecs après coup est correct.
...
`/spectator` est une page authentifiée qui agrège les serveurs dédiés actifs, les sessions en cours et les postes spectateurs avec rafraîchissement périodique et invalidation Socket.IO. Elle renvoie aussi vers la vue TV existante. La capture se fait volontairement depuis le navigateur avec `navigator.mediaDevices.getDisplayMedia` et `MediaRecorder` (Chrome/Edge), puis le fichier WebM est envoyé à l'API à l'arrêt. Le navigateur recharge ensuite le fichier avec Axios authentifié afin que la vidéo ne soit jamais exposée par une URL anonyme.

**`SpectatorController` (`/api/spectator`)**

- `GET /spectator/recordings` (JWT) — métadonnées triées par date.
- `POST /spectator/recordings` (JWT, multipart `file`, `title`, `durationSeconds`) — WebM/MP4/MOV, maximum 500 Mo, stocké dans `ScreenRecording.data` (`bytea`).
- `GET /spectator/recordings/:id/file` (JWT) — lecture inline ou téléchargement avec `?download=1`.
...
## 5.17 Capture Assetto Corsa automatique (v2.2.156)

`LiveCaptureManager` est démarré par `SimRacingAgent.handleLaunch` après un lancement Content Manager/direct et par `handleJoinServer` après une connexion à un serveur. Il est arrêté par `handleStop`, `endSession` et `stop`. Il ne démarre que sur Windows et utilise `ffmpeg.exe` avec `gdigrab` pour capturer le bureau du POD, ce qui suit la fenêtre AC même lorsque Content Manager change de processus ou de titre.

Le flux léger produit des images JPEG (10 fps, largeur 1280) et les envoie authentifiées par clé station à `POST /api/spectator/frame`. Le backend garde uniquement la dernière image par station en mémoire ; `GET /api/spectator/live-sources` liste les sources fraîches et `GET /api/spectator/frame?station=...` sert l'image publique sans cache. `/spectator/screen` fait tourner automatiquement les sources et actualise l'image.

En parallèle, FFmpeg écrit un MP4 H.264 temporaire (30 fps, preset ultrafast). À la fin de la session, l'agent envoie le fichier à `POST /api/spectator/recordings/raw` avec les en-têtes de titre/durée ; il est stocké dans `screen_recordings`. La limite serveur est de 500 Mo. Installer FFmpeg et le rendre accessible par `PATH`, `FFMPEG_PATH` ou un chemin standard (`C:\\ffmpeg\\bin\\ffmpeg.exe`, `Program Files`) sur chaque POD. Une absence de FFmpeg est non bloquante et apparaît dans les logs agent.
...
## 5.22 Fusion des layouts multi-stations (v2.2.167)

`ContentLabelsService.gatherRawContent()` fusionne maintenant les circuits par `acId` entre toutes les stations et déduplique l'union de leurs `layoutNames`. Un inventaire ancien ou partiel ne peut donc plus écraser un layout découvert par un autre poste ; les previews restent résolues depuis la table `content_previews` existante.

## 5.23 Présence du contenu par station (v2.2.168)

`GET /api/content/labels/known` déduplique toujours les voitures et circuits, mais ajoute `stations` et `layoutPresence`. Chaque station est marquée `present: true/false`, ce qui permet à `/content-names` d'expliquer où le contenu et chaque layout sont disponibles sans créer une ligne par poste. Le catalogue public tablette ne reçoit pas ces métadonnées d'administration.

## 5.24 Rescan des layouts après installation (v2.2.169)

`ContentScanner.getTrackScanPaths()` construit l'empreinte du circuit à partir du `ui_track.json`, de la preview et de l'outline du circuit ainsi que de ces fichiers dans chaque dossier de layout (`ui/<layout>`, `<layout>/ui` et `<layout>`). Le cache v10 est invalidé quand un layout est ajouté ou modifié, ce qui évite que le créateur de serveur reçoive un circuit avec `layouts: []` après l'installation d'un nouveau tracé.
