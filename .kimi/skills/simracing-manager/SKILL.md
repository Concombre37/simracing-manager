...
Connaissance complète et exhaustive du monorepo `simracing-manager`, à jour au **`v2.2.119`**. Ce fichier est la source de vérité du projet — le tenir à jour à chaque changement d'architecture, d'endpoint, de contrat WebSocket, de build ou de déploiement.
...
- **Version de référence**: l'agent (`apps/agent/package.json`) — `2.2.119`. Les autres `package.json` (`root`, `backend`, `frontend`, `shared`) restent à `2.2.14` et ne sont **pas** des indicateurs fiables de version produit.
...
- **`ContentLabel`** (v2.2.68) : `id, type ('car'|'track'), acId, displayName, category? (texte libre, ex "GT3", v2.2.117), difficulty? (1-5, v2.2.117), year? (v2.2.118), country?/countryCode? (nom + ISO 3166-1 alpha-2 pour l'emoji drapeau, v2.2.118), description? (v2.2.118), createdAt, updatedAt`, unique sur `(type, acId)`. **Global** (pas de FK station — un `acId` désigne le même contenu AC partout), contrairement à `ContentPreview` qui est scopé par station. Tous ces tags alimentent la page tablette (`/tablet-menu`) — pas de valeur inventée, `null` tant que l'admin n'a rien renseigné via `/content-names`. **259 des 346 voitures scannées en prod ont été peuplées en masse le 2026-08-11** (recherche des vraies données par catégorie/année/pays/description, difficulté estimée par catégorie) via un script Prisma one-off — volontairement incomplet, les mods dont le constructeur est déguisé pour raison de licence (contenus RSS "Bayer"/"Lanzo"/"Protech"...) ont été exclus plutôt que devinés.
...
- **`ContentNames.tsx`** (`/content-names`, admin, v2.2.68, étendu v2.2.117 puis v2.2.118) : en plus du nom personnalisé, chaque ligne a un champ **catégorie** (texte libre), un sélecteur **difficulté** (5 barres cliquables, 1-5, cliquer la valeur déjà sélectionnée l'efface), et depuis v2.2.118 une seconde ligne avec **année**, **pays** (texte libre) + **code pays** (2 lettres ISO, affiche l'emoji drapeau live via `flagEmoji()` — combinaison Unicode de deux "regional indicator symbols", pas d'image), et **description** courte. Un seul bouton "Enregistrer" par ligne envoie tous les champs ensemble (`ContentNameRow`, state local par champ) — nécessaire depuis que `upsert()` ne supprime plus la ligne sur un nom vide seul (voir 3.1/3.3). Tous ces champs alimentent la page tablette publique `/tablet-menu`.
