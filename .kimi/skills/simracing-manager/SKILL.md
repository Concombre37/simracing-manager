...
Connaissance complète et exhaustive du monorepo `simracing-manager`, à jour au **`v2.2.135`**. Ce fichier est la source de vérité du projet — le tenir à jour à chaque changement d'architecture, d'endpoint, de contrat WebSocket, de build ou de déploiement.
...
- **`TabletMenu.tsx`** (`/tablet-menu`, **page publique sans compte**, v2.2.117, enrichie v2.2.118, thème "Nocturne" remplacé par l'identité réelle du site vitrine en v2.2.134) : réimplémentation React d'une maquette Claude Design importée. 4 onglets (Voitures/Circuits/Cuisine/Bar), filtres catégorie **dynamiques depuis v2.2.132** (générés depuis `GET /external/v1/categories`, matché exact sur `item.category` — la barre ne s'affiche que s'il y a au moins un item pour une catégorie donnée), fiche détail au clic (photo + catégorie + difficulté + depuis v2.2.118 pays/drapeau/année/description quand renseignés — **pas** de puissance/poids inventés), écran de veille après `IDLE_MS` (90s) d'inactivité (accroche tournante calculée depuis les vrais compteurs), portrait/paysage détecté via `window.innerWidth`/`innerHeight` (`vh > vw*1.05`). **Depuis v2.2.134**, thème repris de l'identité réelle du site vitrine du client (`elsass-simracing.fr`) plutôt que de la maquette générique d'origine : fond anthracite `#0E0E0E`/`#242424`, accent bleu acier `#245E97`, police Montserrat — tokens CSS scopés à `.tablet-menu` (`TabletMenuStyles`), jamais sur `:root`, pour ne pas teinter le reste du dashboard. Navigation en barre basse unique dans les deux orientations (plus de rail latéral en paysage). **Pas de toggle FR/EN** — tout le reste du site est 100% français sans infra i18n. Consomme `GET /external/v1/content`, `/categories` et `/menu` via `services/tabletMenu.ts` → `services/externalApi.ts` (voir gotcha ci-dessous). Wordmark d'établissement codé en dur (`VENUE_WORD_1`/`VENUE_WORD_2`/`VENUE_CITY`, "ELSASS"/"SIMRACING"/"HAGUENAU") — pas de compte utilisateur sur cette page donc pas d'accès à `useSiteLogo()` (endpoint JWT/clé station uniquement).
  - **v2.2.134, demandé par l'utilisateur ("j'aimerais refaire la page du /tablet-menu... j'aimerais avoir le maximum de beauté [du vrai site elsass-simracing.fr]")** : nouveau prompt de conception écrit à partir d'un rendu Playwright du vrai site (couleurs/police extraites du CSS calculé — `#245E97`/`#242424`/Montserrat — plutôt que devinées), nouvelle maquette Claude Design importée ("Simracing tablet menu", `Kiosque Elsass Simracing.dc.html`) et réimplémentée avec les mêmes écarts assumés que toutes les maquettes précédentes de ce projet : (1) la maquette ajoutait deux onglets fictifs "Arcade" et "Le centre" (bornes d'arcade, photos de salle, événements à venir) sans aucune donnée réelle derrière — **non repris**, contrairement aux 4 onglets réellement demandés dans le prompt de conception ; (2) les circuits de la maquette étaient des tracés SVG inventés avec longueur/virages/sens fictifs — remplacés par le vrai `outline.png` scanné (v2.2.128) sur un panneau noir à halo radial, aucune spec fabriquée ; (3) les drapeaux SVG dessinés à la main pour 9 pays fixes ont été laissés de côté au profit de `flagEmoji()` déjà en place (couvre n'importe quel code pays réel) ; (4) l'écran de veille de la maquette utilisait une photo plein écran du centre (`<image-slot>`, aucun asset réel de ce type côté serveur) — remplacé par un halo radial bleu, sans photo inventée ; (5) les cartes voiture de la maquette étaient volontairement sans photo (dégradé + filigrane de catégorie) — la vraie photo scannée déjà curatée (v2.2.123) a été **conservée** en fond de carte plutôt que supprimée, pour ne pas perdre ce travail ni la reconnaissance visuelle.
