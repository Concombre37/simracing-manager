...
Connaissance complète et exhaustive du monorepo `simracing-manager`, à jour au **`v2.2.117`**. Ce fichier est la source de vérité du projet — le tenir à jour à chaque changement d'architecture, d'endpoint, de contrat WebSocket, de build ou de déploiement.
...
- **Version de référence**: l'agent (`apps/agent/package.json`) — `2.2.117`. Les autres `package.json` (`root`, `backend`, `frontend`, `shared`) restent à `2.2.14` et ne sont **pas** des indicateurs fiables de version produit.
...
- `GET /external/v1/content` (v2.2.117) — `ContentLabelsService.getCatalog()` : catalogue voitures/circuits agrégé sur tous les postes (dédupliqué par `acId`), avec image (`ContentPreview`) et tag catégorie/difficulté (`ContentLabel`). Consommé par `/tablet-menu`.
...
- `GET /content/labels/known` (admin) — agrège tous les `acId` de voitures/circuits déjà vus dans `Station.content` (toutes stations confondues, dédupliqués), joints avec le `ContentLabel` existant s'il y en a un. Depuis v2.2.117, renvoie aussi `category`/`difficulty`.
...
- `PUT /content/labels` (admin) — upsert `{ type: 'car'|'track', acId, displayName, category?, difficulty? }`. Jusqu'en v2.2.116, `displayName` vide supprimait la ligne (retour au nom technique) ; **depuis v2.2.117, la ligne n'est supprimée que si les trois champs sont vides à la fois** — sinon vider juste le nom personnalisé effacerait aussi une catégorie/difficulté déjà renseignée (`ContentLabelsService.upsert()`).
...
- **`ContentLabel`** (v2.2.68) : `id, type ('car'|'track'), acId, displayName, category? (texte libre, ex "GT3", v2.2.117), difficulty? (1-5, v2.2.117), createdAt, updatedAt`, unique sur `(type, acId)`. **Global** (pas de FK station — un `acId` désigne le même contenu AC partout), contrairement à `ContentPreview` qui est scopé par station. `category`/`difficulty` alimentent la page tablette (`/tablet-menu`) — pas de valeur inventée, `null` tant que l'admin n'a rien renseigné via `/content-names`.
...
- **`ContentNames.tsx`** (`/content-names`, admin, v2.2.68, étendu v2.2.117) : en plus du nom personnalisé, chaque ligne a maintenant un champ **catégorie** (texte libre) et un sélecteur **difficulté** (5 barres cliquables, 1-5, cliquer la valeur déjà sélectionnée l'efface). Un seul bouton "Enregistrer" par ligne envoie les trois champs ensemble (`ContentNameRow`, state local `name`/`category`/`difficulty`) — nécessaire depuis que `upsert()` ne supprime plus la ligne sur un nom vide seul (voir 3.1/3.3). Ces deux champs alimentent la page tablette publique `/tablet-menu`.
