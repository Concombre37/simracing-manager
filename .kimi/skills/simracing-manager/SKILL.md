...
- **Current version**: `2.2.40` (agent version is the source of truth; root `package.json` may lag).

### Active agent

| Path            | Role                                                                                                          | Version  | Released?              |
| --------------- | ------------------------------------------------------------------------------------------------------------- | -------- | ---------------------- |
| `apps/agent/`   | New monorepo agent, uses `@simracing/shared`, auto-provisioning + API-key auth, koffi shared-memory telemetry | `2.2.40` | Released               |
...
- **Only one telemetry source may drive blanking's ready-detection at a time** (fixed v2.2.40). `AcSharedMemoryReader` (native, koffi) and the CSP UDP/file fallback (`telemetryReceiver.ts`/`telemetryFileReader.ts`, fed by `lua_app/SimCenterManager/SimCenterManager.lua`) both used to call `blankingManager.onTelemetry()` whenever both were running (which is normal — the fallback runs continuously from agent start, the shared-memory reader runs whenever a session is active). `updateReadyState()` resets its 5s ready-confirmation countdown on _any_ single "not ready" snapshot, so if the two sources' independently-computed `isSessionStarted`/`isInMainMenu` ever disagreed even briefly, the countdown never completed uninterrupted and blanking could stay up indefinitely despite the car genuinely being on track — this was very likely the real, final cause behind the long-running "blanking never clears on a dedicated-server join" saga (v2.2.29 through v2.2.40 all chased pieces of this same symptom). `agent.ts#onTelemetrySnapshot(snapshot, fromSharedMemory)` now only forwards to blanking when `fromSharedMemory` is true, or when `acSharedMemoryReader.isActive()` is false (i.e. shared memory unavailable, e.g. Linux dev or koffi failure) — so exactly one source is ever authoritative.
- **Known dead-code cleanup (v2.2.40)**: `BlankingManager`'s `driving`/`isDriving()`/`lastTelemetryAt`/`telemetryTimeoutMs` were computed but never read anywhere — removed. `HeartbeatPayload.cmRunning`/`vrConnected` were always `false` and never consumed by backend or frontend — removed from the shared contract and the agent. `AcSharedMemoryChecker` (`acSharedMemory.ts`, PowerShell-based polling every 2s) looks redundant with `AcSharedMemoryReader` but isn't: it's the only signal for `acLoaded` **outside** an agent-managed session (e.g. someone launches AC manually without going through the dashboard) — don't remove it without replacing that use case first.
