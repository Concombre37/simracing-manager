...
- **Version de référence**: l'agent (`apps/agent/package.json`) — `2.2.171`. Les autres workspaces suivent désormais la même version de release.
...
- `GET /external/v1/content` (v2.2.117, champs étendus v2.2.118) — `ContentLabelsService.getCatalog()` : catalogue voitures/circuits agrégé sur tous les postes (dédupliqué par `acId`), avec image (`ContentPreview`) et tags (`ContentLabel`). Consommé par `/tablet-menu`.
