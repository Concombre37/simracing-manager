...
Connaissance complète et exhaustive du monorepo `simracing-manager`, à jour au **`v2.2.113`**. Ce fichier est la source de vérité du projet — le tenir à jour à chaque changement d'architecture, d'endpoint, de contrat WebSocket, de build ou de déploiement.
...
- **Version de référence**: l'agent (`apps/agent/package.json`) — `2.2.115`. Les autres `package.json` (`root`, `backend`, `frontend`, `shared`) restent à `2.2.14` et ne sont **pas** des indicateurs fiables de version produit.
...
- Surface en lecture seule pour un consommateur externe (site web, bot Discord...), authentifiée par clé API dédiée (`ExternalApiKeyGuard`, en-tête `X-Api-Key` ou `Authorization: Bearer`) — **jamais** le JWT dashboard.
...
- **`CreateDedicatedServer.tsx`** (wizard 3 étapes : Simulateur → Circuit → Configuration) :
  ...
  - **Sélecteur "Format de course"** (v2.2.115, étape 3) : `Select` toujours visible dans le panneau récapitulatif (pas replié dans "Options avancées" — champ obligatoire, pas secondaire), rempli via `GET /race-formats`, présélectionné sur le premier format de la liste dès qu'elle charge. `raceFormatId` fait partie des conditions de `canSubmit` au même titre que le nom/circuit/voitures.
