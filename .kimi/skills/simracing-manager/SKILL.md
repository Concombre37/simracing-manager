...
Connaissance complète et exhaustive du monorepo `simracing-manager`, à jour au **`v2.2.116`**. Ce fichier est la source de vérité du projet — le tenir à jour à chaque changement d'architecture, d'endpoint, de contrat WebSocket, de build ou de déploiement.
...
- **Version de référence**: l'agent (`apps/agent/package.json`) — `2.2.116`. Les autres `package.json` (`root`, `backend`, `frontend`, `shared`) restent à `2.2.14` et ne sont **pas** des indicateurs fiables de version produit.
