...
async function isSteamRunning(): Promise<boolean> {
  ...
  try {
    const { stdout } = await execAsync(
      'powershell.exe -NoProfile -Command "Get-Process steam -ErrorAction SilentlyContinue | Select-Object -First 1"',
    );
...
async function ensureSteamRunning(logPath: string): Promise<void> {
  ...
  if (!steamExe) {
    console.warn(
      "[joinServer] Steam.exe non trouvé, impossible de le démarrer automatiquement",
    );
    return;
  }
  ...
  try {
    fs.appendFileSync(
      logPath,
      `[${new Date().toISOString()}] Démarrage de Steam : ${steamExe}\n`,
    );
  } catch {}
    ...
    if (fs.existsSync(dir)) {
      try {
        ...
        const found = entries.find((e) =>
          /^Content Manager\s*.+\.exe$/i.test(e),
        );
...
function findDriveKeyHelperPath(): string | null {
  const candidates = [
    path.join(config.baseDir, "tools", "PressDriveKey.exe"),
...
function scheduleDriveKeyPress(logPath: string): void {
  ...
  const helperPath = findDriveKeyHelperPath();
  if (!helperPath) {
    console.log(
      "[joinServer] PressDriveKey.exe non trouve, fallback sur le script PowerShell",
    );
    try {
      fs.appendFileSync(
        logPath,
        `[${new Date().toISOString()}] PressDriveKey.exe non trouve, fallback PowerShell\n`,
      );
    } catch {}
    schedulePowerShellDriveKeyPress(logPath);
    return;
  ...
  console.log(
    `[joinServer] Utilisation du helper PressDriveKey : ${helperPath}`,
  );
  try {
    fs.appendFileSync(
      logPath,
      `[${new Date().toISOString()}] Lancement PressDriveKey : ${helperPath}\n`,
    );
  } catch {}
  ...
  const child = spawn(
    ...
    [
      ...
      "Assetto Corsa",
      "--delay",
      String(config.autoDriveDelayMs),
      "--press",
      String(config.autoDrivePressMs),
      "--repeat",
      String(config.autoDriveRepeat),
      "--interval",
      String(config.autoDriveIntervalMs),
      "--log",
      logPath,
    ],
    {
...
function schedulePowerShellDriveKeyPress(logPath: string): void {
...
`;
  try {
    ...
    console.warn(
      "[joinServer] Impossible de planifier l appui touche:",
      err.message,
    );
  }
}
...
export async function joinServer(cfg: JoinServerConfig): Promise<void> {
  // S'assurer que Start/Restart est mappé sur le bouton A de la manette virtuelle
  // avant qu'AC ne charge et n'affiche l'ecran "volant rouge".
  if (process.platform === "win32" && config.autoMapAcControls) {
    try {
      await ensureStarterBoundToA();
    } catch (err: any) {
      console.warn(`[joinServer] Impossible de verifier le mapping AC : ${err.message}`);
    }
  }

  const raceIniPath = path.join(
    config.documentsPath,
    "Assetto Corsa",
    "cfg",
    "race.ini",
  ...
  if (config.launchMode === "cm" && cmExe) {
    ...
    try {
      fs.appendFileSync(
        logPath,
        `[${new Date().toISOString()}] CM URI: ${uri}\n`,
      );
    } catch {}
  ...
  if (!exe) {
    throw new Error(
      `Impossible de trouver AssettoCorsa.exe, acs.exe ou Content Manager pour lancer AC`,
    );
  }
  ...
  if (isWindows) {
    ...
    child.on("exit", (code) => {
      try {
        fs.appendFileSync(
          logPath,
          `[${new Date().toISOString()}] PowerShell exit code: ${code}\nstderr: ${stderr}\n`,
        );
      } catch {}
    });
    child.unref();
  ...
  console.log(
    `[joinServer] Lancement de ${exe} /spawn pour rejoindre ${cfg.serverIp}:${cfg.serverPort}`,
  );
}
