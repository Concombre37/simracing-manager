...
export function Spectator() {
  ...
  const spectateMutation = useMutation({
    ...
    onError: (error: unknown) => {
      ...
      setSpectatorLaunchMessage(null);
      setSpectatorLaunchError(
        Array.isArray(responseMessage)
          ? responseMessage.join(', ')
          : responseMessage ?? 'Impossible de lancer le poste spectateur.',
      );
    },
  });
          ...
          <section className="rounded-xl border border-white/10 bg-dark-900/60 p-5">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-gray-500">Clique sur un serveur pour y envoyer le poste spectateur. Il utilise toujours la voiture du slot 1.</p>
              </div>
                ...
                return (
                  <div key={server.id} className="flex flex-wrap items-center gap-3 rounded-lg border border-emerald-500/20 bg-emerald-500/[0.06] p-3">
                    ...
                    >
                      ...
                      {spectateMutation.isPending ? 'Connexion…' : 'Lancer le spectateur'}
                    </button>
            ...
            {spectatorLaunchMessage && <p className="mt-3 rounded-md border border-emerald-400/25 bg-emerald-400/10 px-3 py-2 text-xs text-emerald-200">{spectatorLaunchMessage}</p>}
            {spectatorLaunchError && <p className="mt-3 rounded-md border border-red-400/25 bg-red-400/10 px-3 py-2 text-xs text-red-200">{spectatorLaunchError}</p>}
          </section>
