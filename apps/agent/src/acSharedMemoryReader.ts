...
interface SharedMemoryTypes {
  SPageFilePhysics: ReturnType<typeof import('koffi').pack>;
  SPageFileGraphic: ReturnType<typeof import('koffi').pack>;
  SPageFileStatic: ReturnType<typeof import('koffi').pack>;
}

/** Registered once at module load, not per AcSharedMemoryReader instance.
 * koffi's type registry is global to the process — a second instance
 * (e.g. a fresh reader created on every backend reconnect, see
 * agent.ts's socket 'connect' handler) re-registering the same struct
 * names used to throw "Duplicate type name 'SPageFilePhysics'", which
 * the constructor's try/catch only ever logged and swallowed — silently
 * breaking telemetry for the rest of the agent's life after the very
 * first reconnect. */
let sharedMemoryTypes: SharedMemoryTypes | null = null;
let sharedMemoryTypesError: unknown = null;

if (koffi) {
  try {
    const SPageFilePhysics = koffi.pack('SPageFilePhysics', {
      packetId: 'int32',
      gas: 'float',
      brake: 'float',
      fuel: 'float',
    ...
    const SPageFileGraphic = koffi.pack('SPageFileGraphic', {
      packetId: 'int32',
      status: 'int32',
      session: 'int32',
    ...
    const SPageFileStatic = koffi.pack('SPageFileStatic', {
    ...
    sharedMemoryTypes = { SPageFilePhysics, SPageFileGraphic, SPageFileStatic };
  } catch (err) {
    sharedMemoryTypesError = err;
  }
}
...
export class AcSharedMemoryReader {
  ...
  constructor(
    ...
    if (!sharedMemoryTypes) {
      this.logger.error(
        { err: sharedMemoryTypesError },
        'Failed to initialize koffi shared memory bindings',
      );
      return;
    }
    this.SPageFilePhysics = sharedMemoryTypes.SPageFilePhysics;
    this.SPageFileGraphic = sharedMemoryTypes.SPageFileGraphic;
    this.SPageFileStatic = sharedMemoryTypes.SPageFileStatic;
    ...
    try {
      ...
      this.logger.error({ err }, 'Failed to initialize koffi kernel32 bindings');
    }
  }
