...
function getAutoStartFlagPath(): string {
  return path.join(config.documentsPath, 'Assetto Corsa', 'cfg', 'simcenter_autostart.flag');
}
...
function scheduleDriveKeyPress(logPath: string): void {
  ...
  if (!helperPath) {
    console.log('[joinServer] PressDriveKey.exe non trouve, fallback sur le script PowerShell');
    try {
      fs.appendFileSync(
        logPath,
  ...
  console.log(`[joinServer] Utilisation du helper PressDriveKey : ${helperPath}`);
  try {
    fs.appendFileSync(
      logPath,
