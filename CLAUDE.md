...
Connaissance complète et exhaustive du monorepo `simracing-manager`, à jour au **`v2.2.74`**. Ce fichier est chargé automatiquement par Claude Code (contexte de projet) et sert de source de vérité — le tenir à jour à chaque changement d'architecture, d'endpoint, de contrat WebSocket, de build ou de déploiement.
...
- **Crash _instantané_ (< 1s, code 2) distinct, spécifique au mix de plusieurs voitures — corrigé en v2.2.74.** Signalé par l'utilisateur en testant pour la première fois en conditions réelles la fonctionnalité de mix de voitures (grille de sélection ajoutée cette session). `writeServerConfig()` écrivait `CARS=` (le champ qui liste les modèles **distincts** autorisés sur le serveur) directement à partir de `payload.cars`, le tableau brut par-emplacement — qui peut désormais contenir des doublons (une voiture choisie 3 fois → 3 entrées identiques) depuis que le sélecteur permet de mélanger plusieurs voitures au lieu d'en imposer une seule pour tous les emplacements. `acServer.exe` rejetait ce `CARS=` dupliqué et quittait avant même de tenter d'ouvrir son port UDP — logs agent : `Server process exited (code=2)` moins d'une seconde après le lancement, deux fois de suite avec deux serveurs différents. `entry_list.ini` (l'assignation par emplacement, où les doublons sont légitimes et attendus) n'était pas concerné. Fix : `CARS=` est maintenant dédupliqué (`[...new Set(carIds)]`) avant écriture, `entry_list.ini` inchangé.
