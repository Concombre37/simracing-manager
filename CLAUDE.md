...
Project-scope knowledge for working on the `simracing-manager` monorepo. Carried over from the project's previous `SKILL.md` (maintained under `.kimi`/`.kimi-code`) so Claude Code has the same context going forward.
...
- **Remote agent update**: technicians and admins can trigger an agent update from the Stations page. The agent downloads `sim-center-agent-win.zip` from the latest GitHub release and replaces itself (including koffi native binaries). `update-agent.ps1` (v2.2.65) backs up the current exe/`build/` folder before extracting and restores it if `Expand-Archive` fails, always attempting a relaunch either way (new version on success, previous one on failure) rather than leaving the agent stopped — steps logged to `update-agent.log` next to the executable. Even so, a truly-failed relaunch (or any other unexpected crash) still needs a watchdog to notice, see below.
- **Watchdog process (v2.2.67, `watchdogManager.ts` + `assets/watchdog.ps1`)**: a separate detached PowerShell process, started by `agent.ts#start()` (`ensureRunning()`, skipped if one is already alive — tracked via a PID file at `<tmp>/simracing-manager/watchdog.pid`, confirmed alive with `tasklist`), that checks every 20s whether the agent's process is running and relaunches it (via `start-agent.vbs`) after a 15s grace period if not — the real safety net for a crash or an update whose own relaunch step also failed. It has to be a genuinely separate process: if the agent itself died, it can't be the one noticing. Explicitly stopped (by PID, via `taskkill`) before every _deliberate_ exit (`agent.stop()`, `handleUpdate()`, `handleLocalRestart()`) so it never races a legitimate shutdown/update by relaunching over it — the next `start()` re-establishes it once the new/restored process is up. Logs to `watchdog.log` next to the executable.
- **`handleLocalRestart()` (console "Redémarrer l'agent") used the same cmd.exe delayed-expansion bug already fixed once in the updater (v2.2.61)** — `set /a waitTime+=1` inside a parenthesized `if (...)` block never actually incremented within the same iteration. Fixed the same way: a generated PowerShell script using `Wait-Process`, relaunching via `start-agent.vbs` when present (previously always flashed a console window via a direct `start`).
...
- Update this file (and `CHANGELOG.md`) if you change architecture, build steps, or deployment.
